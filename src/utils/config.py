...
class ProjectConfig:
    ...
    # Обучение (оптимизировано для скорости и качества)
    epochs: int = 100  # Еще больше эпох для сложных случаев
    img_size: int = 800  # Оптимизировано для скорости (было 1280, теперь 800 для ~15-20 it/s вместо 6)
    batch_size: int = 4  # Увеличено для лучшей утилизации GPU (было 2, теперь 4 для скорости)
